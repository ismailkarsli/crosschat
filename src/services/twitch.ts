...
const { TWITCH_BOT_USERNAME, TWITCH_BOT_ACCESS_TOKEN } = process.env;
...
export default class Twitch extends ServiceEmitter {
  tmiClient: Client | undefined;
  private channelName!: string;
  constructor() {
    super();
  }
  static async init({ channelName }: { channelName: string }) {
    const instance = new Twitch();
    instance.channelName = channelName;

    instance.tmiClient = new tmi.Client({
      options: {},
      identity: {
    ...
    instance.tmiClient.connect().catch(console.error);

    instance.tmiClient.on("message", (channel, tags, message, self) => {
      if (self) return;
